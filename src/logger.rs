...
const LOG_FILE: &str = "email.log";
const PREFIX: &str = "[Email]";

/// Console-only sink. Kept as its own `Box<dyn Log>` rather than installed
/// globally, so a message can reach the console without also reaching
/// `logs/email.log` — the split `error_detail` relies on to keep sensitive
/// detail out of the console.
...
impl Logger {
    pub fn init() {
        ...
        // `logs/email.log` — size-based rotation into `logs/archive/`, its own
        ...
        // `EMAIL_SAMP_LOG_*` without a rebuild.
        let installed = samp::enable_logger_with!(
            LoggerConfig::new(env!("CARGO_PKG_NAME"))
                ...
                // an operator can turn it off with EMAIL_SAMP_LOG_COMPRESS=0.
                .compress_archives(true)
                .from_env()
                // After `from_env` on purpose: `EMAIL_SAMP_LOG_SERVER=1` would
    ...
    /// banner honours `EMAIL_SAMP_LOG_LEVEL` too.
