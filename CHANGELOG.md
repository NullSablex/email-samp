...
- **`email_errno` / `email_error`** — `0` is the last failure on any account, which is also where a failed `email_setup` or `email_connect` lands; an account id reads that account's own.
